...
var OAuthAdapter = function(pConsumerSecret, pConsumerKey, pSignatureMethod, params) {
    ...
    var sendByFormData = function(params) {
        var pUrl = params.url;
        ...
        for (var i = 0; i < pParameters.length; i++) {
            var name = pParameters[i][0];
            var value = pParameters[i][1];
            
            body.write(String.format("--%s\r\n", boundary), true);
            
            switch (typeof value) {
                case 'object':
                    if (value.toString() == "[object TiBlob]") {
                        body.write(String.format("Content-Disposition: form-data; name=\"%s\"; filename=\"%s\"\r\n", name, "image.png"), true);
                        body.write("Content-Type: image/png\r\n", true);
                        body.write("Content-Transfer-Encoding: binary\r\n", true);
                        body.write("\r\n", true);
                        body.write(value, true);
                        body.write("\r\n", true);
                    }
                    break;
                default:
        ...
        client.onerror = function(e) {
            if(params.onError){
                params.onError({error:'[' + client.status + '] ' + e.message});
            }
        }
        client.open(pMethod,pUrl,false);
        client.setRequestHeader('Content-Type', String.format('multipart/form-data; boundary=%s;', boundary));
        var body = Ti.Filesystem.getFile(Ti.Filesystem.tempDirectory, filename);
        client.send(body.read());
    }
    this.sendByFormData = sendByFormData;
