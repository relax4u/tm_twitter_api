...
var TwitterApi = function(params) {
    ...
    this._callApi = function(params) {
		...
		for (var index in params){
			...
			if ('sendByFormData' == index) continue;
			if ('parameters' == index) {
        ...
        var methodName = params.sendByFormData ? "sendByFormData" : "send";
        
        var response = oAuthAdapter[methodName]({
            url: params.url,
    ...
    */
    ...
    // api.statuses_update_with_media({
    //     status: "Tweet message",
    //     'media[]': TiBlob(png image data)
    // })
    this.statuses_update_with_media = function(params) {
        params.url = 'https://upload.twitter.com/1/statuses/update_with_media.json'
        params.method = "POST";
        params.sendByFormData = true;
        return self.callApi(params);
    };
    //statuses.destroy
